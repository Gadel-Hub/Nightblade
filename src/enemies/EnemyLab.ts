...
export class EnemyLab {
  ...
  constructor(private readonly scene: Phaser.Scene, private readonly terrain: Phaser.Physics.Arcade.StaticGroup) {
    ...
    for (const [index, x] of this.bayStarts.entries()) {
      ...
      scene.add.text(x + 32, 380, `${index + 1} ${['GUARD', 'RANGED'][index]} / NUMBER KEY RESETS`, { fontFamily: 'monospace', fontSize: '8px' });
    }
  }
  ...
  reset(): void {
    ...
    if (this.selected === 1) this.enemies.push(new RangedAttacker(this.scene, this.terrain, 2768, 436,
      new Phaser.Geom.Rectangle(2480, 0, 448, 540)));
  }
